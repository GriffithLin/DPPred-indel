...
def train(args, paths=None):
    ...
    for counter in range(args.model_num):
        ...
        source_classifier = TextCNN_confusion_Embedding_unstudent(64, args.filter_size_dna, args.filter_size_protein,
                                  args.output_size, args.dropout)

        print(source_classifier)
        source_classifier.load_state_dict(torch.load(args.model_path), strict=False)
        ...
        if args.opt == "SGD":
            optimizer = torch.optim.SGD(target_classifier.parameters(), lr=args.learning_rate, momentum=args.momentum,
                                        weight_decay=args.weight_decay, nesterov=True)  # 优化�?
        else:
            optimizer = torch.optim.Adam(target_classifier.parameters(), lr=args.learning_rate)  # 优化�?
        ...
        # 初始化训练类
        Train = DataTrain_confusion(source_classifier, target_classifier, optimizer, criterion, criterion_cont, lr_scheduler, device=DEVICE)

        domain_discri = DomainDiscriminator(in_feature=64, hidden_size=512).to(DEVICE)
        ...
        torch.save(target_classifier.state_dict(), each_model)
...
if __name__ == '__main__':
    ...
    for parse.batch_size in batch_size_list:
        parse.model_name = "DA_DANN_textCNN_dropout0.6_100_lr0.0002_64_student_trade_off1_BS" + str(parse.batch_size)
