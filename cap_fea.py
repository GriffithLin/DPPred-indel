...
from train import predict, CosineScheduler, DataTrain_confusion, predict_confusion, predict_confusion_cap
from my_util import get_config, data_load_npy_confusion_cont, data_load_npy, data_load_npy_confusion_predict, \
    data_load_npy_predict, save_results, print_score, spent_time, data_load_npy_confusion_cont_cap
from models.model import TextCNN, TextCNN_confusion, TextCNN_CLS, TextCNN_confusion_Contrastive, ContrastiveLoss, TextCNN_confusion_cap
...
def train(args, paths=None):
    ...
    if args.confusion == True:
        train_dataset, dataset_train_cont, test_dataset = data_load_npy_confusion_cont_cap(args.data_direction, args.dna_data_direction, args.batch_size)
    else:
    ...
    for counter in range(args.model_num):
        ...
        if args.Contrastive:
            ...
            model = TextCNN_confusion_cap(args.filter_num, args.filter_size_dna, args.filter_size_protein,
                                      args.output_size, args.dropout)
        else:
        ...
        if args.Contrastive:
            ...
            Train.train_step_cap(train_dataset, test_dataset, args.model_name, epochs=args.epochs, model_num=counter,
                             early_stop=args.early_stop, threshold=args.threshold)
        ...
        # 模型预测
        if args.confusion == True:
            model_predictions, true_labels = predict_confusion_cap(model, test_dataset, device=DEVICE)
        else:
...
if __name__ == '__main__':
    ...
    parse.model_name = "textCNN_dropout0.7_60_"
    path = []
    parse.do_train = True
