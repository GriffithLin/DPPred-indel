...
from train import predict, CosineScheduler, DataTrain_confusion, predict_confusion_pad
...
#     dataset_train = NpyDataset_confusion(train_protein_data, train_dna_data, train_label, data_protein_len_path)
...
def train(args, paths=None):
    ...
    for counter in range(args.model_num):
        ...
        if args.Contrastive:
            ...
            Train.train_step_batch_pad(train_dataset, test_dataset, args.model_name, epochs=args.epochs, model_num=counter,
                             early_stop=args.early_stop, threshold=args.threshold)
        ...
        # 模型预测
        if args.confusion == True:
            model_predictions, true_labels = predict_confusion_pad(model, test_dataset, device=DEVICE)
        else:
...
if __name__ == '__main__':
    ...
    parse.model_name = "textCNN_dropout0.6_batch_pad"
    path = []
    parse.do_train = True
