...
dataset = FastaBatchedDataset.from_file("/data3/linming/DNA_Lin/dataCenter/DA_target_test.fasta")
data_loader = torch.utils.data.DataLoader(
    dataset, collate_fn=batch_converter, shuffle=False
)

# 定义保存文件的路径
representations_file = '/data3/linming/DNA_Lin/dataCenter/DA_target_test.npy'
labels_file = '/data3/linming/DNA_Lin/dataCenter/DA_target_test_labels.npy'
...
with torch.no_grad():
    for batch_idx, (labels, strs, toks) in enumerate(data_loader):
        ...
        for i, tokens_len in enumerate(batch_lens):
            ...
            output_fea_file = os.path.join("/data3/linming/DNA_Lin/dataCenter", "temp", fileName)

            np.save(output_fea_file, sequence_representation)
            j += 1
...
df_data.to_csv("/data3/linming/DNA_Lin/dataCenter/DA_target_test_list.csv")
# # 如果你想保存标签名
