...
from my_util import get_config, data_load_npy_confusion_cont_k_fold, data_load_npy, data_load_npy_predict, \
    data_load_npy_confusion_predict, save_results, print_score, spent_time
...
def train(args, paths=None):
    ...
    if args.confusion == True:
        train_datasets, train_datasets_cont, dev_datasets, test_dataset2 = data_load_npy_confusion_cont_k_fold(args.data_direction, args.dna_data_direction, args.k_fold, args.batch_size)
    else:
    ...
    # for test_dataset in dev_datasets:
        ...
        if args.divide_validata == True:
            model_predictions, true_labels = predict_confusion_DA(model, test_dataset2, device=DEVICE)
...
def do_predict(args):
    ...
    print("data loading....")
    if args.confusion:
        train_datasets, train_datasets_cont, test_datasets, test_dataset2 = data_load_npy_confusion_cont_k_fold(
...
if __name__ == '__main__':
    ...
    for k_mer in k_mer_list:
        for crop_len in crop_len_list:
            ...
            print(parse)
            for parse.k_fold in [0]:
            # for parse.k_fold in [1, 3, 4, 0, 2]:
                parse.model_name = parse.model_name  + "1"
                # if parse.do_train:
                #     train(parse)
                if parse.do_predict:
                    PATH = os.getcwd()
